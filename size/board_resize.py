# https://github.com/leela-zero/leela-zero/issues/2240#issuecomment-466739011
...
if len(sys.argv)==1:
    print("weight tools - change size")
    print("Usage:")
    print("python3 board_size.py original_weight targer_size")
    sys.exit()
...
#zliu value head from the last 4th line
...
#zliu value head reshape to 256 x 361
val_wts = val_wts.reshape(val_filters, num_inters)
#zliu what's val_lens
...
def single_step_mat(size, stride): # 子块法
...
def single_step_fc_mat(size, stride):
    assert stride, "Error: stride cannot be 0."
...
def iter_mat(orig_size, target_size, stride, fc=False): # 逐次子块法
...
def rescale_mat(orig_size, target_size, stride, edgewt=1): # 缩图法
    # stride = 2 means even/odd parity / checkerboard / 网格式
    assert stride > 0, "Error: stride must be positive."
    assert orig_size != target_size, "Error: size must be changed."
    assert (orig_size - target_size) % stride == 0, "Error: size difference must be a multiple of stride."
    rows = []; cols = []; data = [];
...
val_w_mat = iter_mat(orig_size,target_size,2)
...
# new_val_wts = val_w_mat * val_wts * val_w_mat.transpose()
...
# print(np.array([[1,1],[2,2]])/np.array([3,4]))
