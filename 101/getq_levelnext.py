...
def get_nextnum(soup, url_level, url_no):
    ...
    match = re.match(r'^/([^/]+)/([^/]+)/$', next_url)
    if match:
        next_url_level = match.group(1)   # '5D'
        next_url_no = match.group(2)      # '163739'
    else:
        next_url_level = None
        next_url_no = None

    prev_link_element = breadcrumb_ul.find('a', id='prevq')
    prev_url = prev_link_element['href'] if prev_link_element else None
    match = re.match(r'^/([^/]+)/([^/]+)/$', prev_url)
    if match:
        prev_url_level = match.group(1)
        prev_url_no = match.group(2)
    else:
        prev_url_level = None
        prev_url_no = None

    # 输出结果
    print(f'{url_level} {url_no} 第{q_num}题')
    print(f'上一题: {prev_url_level} {prev_url_no}')
    print(f'下一题: {next_url_level} {next_url_no}')
...
def getq_levelnext(username, url_level, url_no):
    ...
    if session:
        ...
        if response:
            ...
            #解析当前页面的上一题，下一题
            prev_url_level, prev_url_no, next_url_level, next_url_no = get_nextnum(soup, url_level, url_no)

            #上一题:插入level表格
            insert_level_urlno(prev_url_level, prev_url_no)

            #解析当前题目
            ret = resp_json(response, level_str, no)
            if ret.get('ret'):
                update_q(ret.get('data'))
                return {'ret': True, 'data': ret.get('data'), 'next_url_level': next_url_level, 'next_url_no':next_url_no}
            else:
...
if __name__ == "__main__":
    ...
    while ret.get('next_url_level') and ret.get('next_url_no'):
        ret = getq_levelnext(ret.get('next_url_level'), ret.get('next_url_no'))
        print()
