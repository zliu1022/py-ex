...
item = ["000000000080200000000",

self.v2_struct = struct.Struct('4s328s162sbB')
stm = int(text_item[16])
...
BOARD_SIZE = 9
INTERSECTIONS = BOARD_SIZE * BOARD_SIZE
POTENTIAL_MOVES = INTERSECTIONS + 1
...
def remap_vertex(vertex, symmetry):
    ...
    """
    assert vertex >= 0 and vertex < INTERSECTIONS
    x = vertex % BOARD_SIZE
    y = vertex // BOARD_SIZE
    if symmetry >= 4:
        x, y = y, x
...
class ChunkParser:
    def __init__(self, chunkdatasrc, shuffle_size=1, sample=1,
        ...
        self.prob_reflection_table = [
            [remap_vertex(vertex, sym)
              for vertex in range(INTERSECTIONS)]+[INTERSECTIONS] for sym in range(8)]
        # Build full 16-plane reflection tables.
        self.full_reflection_table = [
            np.array([remap_vertex(vertex, sym) + p * INTERSECTIONS
                for p in range(16) for vertex in range(INTERSECTIONS)])
                    for sym in range(8)]
        ...
        # self.flat_planes = [ b'\1'*INTERSECTIONS + b'\0'*INTERSECTIONS, b'\0'*INTERSECTIONS + b'\1'*INTERSECTIONS ]
    ...
    def init_structs(self):
        ...
        # uint8 side_to_move (1 byte) -> stm int8
        # uint8 is_winner (1 byte)
        if BOARD_SIZE == 9:
            # (19*19+1)*4=1448, (9*9+1)*4=328
            # 19*19*16/8=722, 9*9*16/8=162
            self.v2_struct = struct.Struct('4s328s162sbB')
        else:
    ...
    def convert_v1_to_v2(self, text_item):
        ...
        if np.any(np.isnan(probabilities)):
            ...
            return False, None
        if not(len(probabilities) == POTENTIAL_MOVES):
            return False, None

        probs = probabilities.tobytes()
        if not(len(probs) == POTENTIAL_MOVES * 4):
            return False, None
    ...
    def v2_apply_symmetry(self, symmetry, content):
        ...
        assert len(planes) == INTERSECTIONS*16
        ...
        assert len(probs) == POTENTIAL_MOVES
        probs = probs.tobytes()
    ...
    def convert_v2_to_tuple(self, content):
        ...
        (ver, probs, planes, to_move, winner) = self.v2_struct.unpack(content)
        # Unpack planes.
        planes = np.unpackbits(np.frombuffer(planes, dtype=np.uint8)).astype('b')
        assert len(planes) == INTERSECTIONS*16
        ...
        planes = planes.tobytes() + (np.array([1.0-stm]*INTERSECTIONS + [stm]*INTERSECTIONS)).astype('b').tobytes()
        assert len(planes) == (18 * INTERSECTIONS), len(planes)
